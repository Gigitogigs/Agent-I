...
async def update_api_key(db: AsyncSession, workspace_id: UUID, agent_type: str, api_key: str, provider: str) -> str:
    # Encrypt the raw key
    encrypted_key = encrypt_secret(api_key)
    
    # Generate hint (e.g. sk-ant-...xyz)
    prefix = api_key[:7]
    suffix = api_key[-4:] if len(api_key) > 11 else ""
    ...
    if provider_key:
        provider_key.encrypted_api_key = encrypted_key
        provider_key.key_hint = hint
        provider_key.is_verified = True
    else:
    ...
    # Update active provider in WorkspaceAgentConfig
    config = await db.scalar(select(WorkspaceAgentConfig).where(WorkspaceAgentConfig.workspace_id == workspace_id))
    if not config:
        config = WorkspaceAgentConfig(workspace_id=workspace_id, active_provider=provider)
        db.add(config)
    else:
